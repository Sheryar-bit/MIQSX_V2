    ...
    <section id="pricing" style={{ padding: 'clamp(56px, 8vh, 100px) clamp(20px, 5vw, 60px) clamp(76px, 11vh, 130px)', maxWidth: '1180px', margin: '0 auto' }}>
      ...
      <div className="mqsx-price">
        <div data-reveal style={{ borderRadius: '22px', border: '1px solid var(--border)', background: 'var(--card)', padding: 'clamp(26px, 3vw, 36px)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', marginBottom: '22px' }}><span style={{ fontSize: '13px', color: 'var(--dim)' }}>PKR</span><span style={{ fontFamily: "'Schibsted Grotesk', sans-serif", fontSize: '46px', fontWeight: 600, letterSpacing: '-0.02em' }}>{FREE.price.monthly.toLocaleString()}</span></div>
        ...
        <div data-reveal data-reveal-delay="80" style={{ position: 'relative', borderRadius: '22px', background: 'var(--olive)', color: 'var(--olive-on)', padding: 'clamp(30px, 3vw, 40px) clamp(26px, 3vw, 36px)', display: 'flex', flexDirection: 'column', boxShadow: 'var(--shadow)', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'baseline', gap: '6px', marginBottom: '22px' }}><span style={{ fontSize: '13px', opacity: .75 }}>PKR</span><span style={{ fontFamily: "'Schibsted Grotesk', sans-serif", fontSize: '46px', fontWeight: 600, letterSpacing: '-0.02em' }}>{PRO.price.monthly.toLocaleString()}</span><span style={{ fontSize: '14px', opacity: .75 }}>/mo</span></div>
        ...
        <div data-reveal data-reveal-delay="160" style={{ borderRadius: '22px', border: '1px solid var(--border)', background: 'var(--card)', padding: 'clamp(26px, 3vw, 36px)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', marginBottom: '22px' }}><span style={{ fontSize: '13px', color: 'var(--dim)' }}>PKR</span><span style={{ fontFamily: "'Schibsted Grotesk', sans-serif", fontSize: '46px', fontWeight: 600, letterSpacing: '-0.02em' }}>{AGENCY.price.monthly.toLocaleString()}</span><span style={{ fontSize: '14px', color: 'var(--dim)' }}>/mo</span></div>
