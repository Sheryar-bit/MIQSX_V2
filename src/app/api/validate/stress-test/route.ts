...
export async function POST(req: NextRequest) {
  ...
  const gate = await enforceOrgLimit(session, "stress-test");
  ...
  await trackEvent({ userId: session.user.id, orgId: gate.orgId, feature: "stress-test", event: "stress-test.run", step: 3 });
